...
import { getAllProductsForBasket } from "@services/productService";
...
export default function BasketGrid({ onAdd, onRemove }) {
  ...
  const {
    ...
    queryKey: ["basket-products"],
    queryFn: ({ signal }) => getAllProductsForBasket(signal),
    select: (data) => nullsToZero(data),
    onError: () => {
      toast.error("خطا در دریافت کالاها");
    },
  });
    ...
    <>
      ...
      <div className="gap-3 mb-3">
        ...
        <button
          ...
          disabled={isFetching}
        >
          {isFetching ? "در حال بروزرسانی..." : "رفرش"}
        </button>
      </div>
      ...
      <div className="row">
        {filtered.map((p) => (
          ...
          >
            <div
              className={`dk-card ${
                animateId === p.productId ? "dk-bounce" : ""
              }`}
            >
              {/* Title */}
              <div
              ...
              <img src={mobile} style={{ width: "80px" }} alt="" />
              {/* Price */}
